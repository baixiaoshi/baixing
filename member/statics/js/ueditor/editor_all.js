...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            ci = opt[0];
            if (opt.length == 1) {
                str = '<img id="'+ci.id+'" src="' + ci.src + '" ' + (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src + '" ' : '') +
...
UE.plugins['pagebreak'] = function () {
    ...
    me.ready(function(){
        utils.cssRule('pagebreak','.pagebreak{display:block;clear:both !important;cursor:default !important;width: 100% !important;margin:0;background: whitesmoke; border: 1px solid #CCCCCC;padding: 3px 0;text-align: center;}',me.document);
    });
    function isHr(node){
        return node && node.nodeType == 1 && node.className == 'pagebreak';
    }
        ...
        execCommand:function () {
			...
			throughBox({
				...
				ok: function (here) {
					...
					if (!stitle) stitle = ' ';
					// 处理分页
					var range = me.selection.getRange(),hr = me.document.createElement('div');
